...
namespace TetriNET2.Server.WCFHost
{
    internal class IPFilterServiceBehavior : IDispatchMessageInspector, IServiceBehavior
    {
        ...
        public object AfterReceiveRequest(ref Message request, IClientChannel channel, InstanceContext instanceContext)
        {
            ...
            if (request.Properties[RemoteEndpointMessageProperty.Name] is RemoteEndpointMessageProperty remoteEndpoint)
            {
                // The address is a string so we have to parse to get as a number
                var address = new WCFAddress(IPAddress.Parse(remoteEndpoint.Address));

                // If ip address is denied clear the request mesage so service method does not get execute
                if (_verifier.IsBanned(address))
                {
                    Log.Default.WriteLine(LogLevels.Warning, "Banned player with address {0} tried to connect", address);
